...
开放防火墙端口时，协议选择直接回车会使用推荐的 TCP + UDP；关闭端口时仍默认
TCP，避免误关同时使用 UDP 的服务。
...
同一动作、端口和协议同时存在 IPv4 与 IPv6 规则时，端口列表会合并标为“`双栈`”；
只有一个地址族时则直接标为“`IPv4`”或“`IPv6`”。默认入站策略仍分别显示两个
地址族，以免掩盖实际安全状态。

开放或关闭指定端口时，可以选择 `TCP`、`UDP` 或 `TCP + UDP`。开放端口时默认
推荐并直接选择 `TCP + UDP`；关闭端口时保留更谨慎的 TCP 默认值。开放端口后可以
填写长度不超过 80 个字符的备注，直接回车会保存为“无”。备注会显示在对应的明确
放行端口旁，不会写入或替换 iptables 的规则所有权标记。

每次通过菜单开放或关闭端口，规则列表和菜单之间都会显示最近一次端口操作的时间、
结果、端口、协议和备注。第 10 项可回看最近 20 条操作，包含失败操作，便于确认
刚才输入的内容。备注和历史记录保存在 `/var/lib/safe-ssh-port/`，目录权限为
`0700`、文件权限为 `0600`；它们仅用于展示，实际放行状态仍以当前防火墙规则为准。
